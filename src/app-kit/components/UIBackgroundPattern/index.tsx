  ...
  const styles = React.useMemo(() => {
    switch (pattern) {
      ...
      case BackgroundPattern.DOT:
        return 'absolute inset-0 h-full w-full bg-[radial-gradient(#27272A_1px,transparent_1px)] [background-size:16px_16px] dark:bg-black';
      case BackgroundPattern.MASK:
        return 'absolute inset-0 h-full w-full bg-[radial-gradient(#27272A_1px,transparent_1px)] [background-size:16px_16px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_60%,transparent_100%)] bg-white dark:bg-black';
      default:
        return '';
    }
  }, [pattern]);
